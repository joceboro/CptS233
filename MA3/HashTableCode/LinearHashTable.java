...
CptS 233: MA #3
11/25/2020
gitRepo url:https://github.com/joceboro/CptS233
*/
...
class LinearHashTable<K, V> extends HashTableBase<K, V>
{
    ...
    public void addElement(K key, V value)
    {
        ...
        //if there is another item with the same key, then replace the value
        if (containsElement(key)) {

            hashItem = new HashItem(key, value, false);
            _items.set(hash, hashItem);
            return;

        while (hash < _items.size()){
            
            //If the slot is empty, add the new hashItem to the slot
            if (_items.elementAt(hash)._is_empty) {
    ...
    public void removeElement(K key)
    {
        ...
        //look for the key if the key is not at it's initial hash
        if(_items.elementAt(hash)._key != key) {

            //Look for the key
            for (int i = 0; i < _items.size(); i++) {

                if(_items.elementAt(i)._key == key) {
                    hash = i; 
                    break;
                }
            }
        ...
        //If the item is not already empty, set it to empty
        if(!(_items.elementAt(hash)._is_empty)) {

            hashItem = new HashItem(null, null, true);
            _items.set(hash, hashItem);
            _number_of_elements--;
        } else {
            System.out.println("Item is already empty");
        }
